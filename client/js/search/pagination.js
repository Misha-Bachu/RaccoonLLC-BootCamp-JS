...
function getPageProducts(paginationPage, products) {
    if (!products || !products.length) {
        return [];
    }
    const start = paginationPage * preferences.paginationPageSize;
    const finish = start + preferences.paginationPageSize;

    return products.slice(start, finish);
}
    ...
    if (numberOfPages > 1) {
        for (let i = 0; i < numberOfPages; i += 1) {
            paginationBlock.insertAdjacentHTML('beforeend',
                `<div class="b-page__pagination_element ${i === 0 ? 'active' : ''} js-pagination-element"
                    data-index=${i}
                >
                    ${i + 1}
                </div>`);
